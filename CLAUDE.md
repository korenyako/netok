...
Current version is in `netok_desktop/src-tauri/Cargo.toml`. Format: `MAJOR.MINOR.PATCH`

**Pre-release (`0.x.x`):**
- `0.1.0` → `0.1.1` — Bug fixes, small improvements
- `0.1.x` → `0.2.0` — New features, significant changes
- `0.x.x` → `1.0.0` — First stable public release
...
- Multiple related files have been changed
...
**Examples:**
```
fix(ui): resolve click events in drag region headers
feat(dns): add IPv6 support for custom DNS
feat(about): show version from Tauri API
refactor(screens): unify header structure across all screens
```
