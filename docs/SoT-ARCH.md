...
(опционально) get_version() -> string
...
Заморозить старый Iced-UI (branch legacy-iced).

Выделить netok_core (только логика и DTO).

Добавить netok_bridge (Tauri cmd + uniffi).

Поднять netok_desktop (Tauri) с минимальным экраном.

Вынести тексты в /i18n/*.json, перепривязать ключи.

Включить автообновления, single-instance, системный трэй.

CI-сборки, smoke-тест, пилот на Win + macOS.
...
docs/SoT-ARCH.md exists with the exact content.

docs index links to it.

Deprecation banners in legacy files include a link to docs/SoT-ARCH.md.

Single commit with the provided message.
