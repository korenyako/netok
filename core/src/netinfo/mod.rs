...
pub enum NetworkKind { Wifi, Ethernet, Other }
...
pub fn detect_network_kind() -> NetworkKind {
    ...
    for name in &active_names {
        ...
        if lower.contains("wi-fi") || lower.contains("wifi") || lower.contains("wlan") || lower.contains("wireless") || lower.contains("беспровод") {
    ...
    for name in &active_names {
        let lower = name.to_lowercase();
        if lower.contains("ethernet") || lower.starts_with("en") || lower.starts_with("eth") || lower.contains("realtek") || lower.contains("intel") {
...
pub fn wifi_signal_dbm() -> Option<i32> {
    #[cfg(target_os = "windows")]
    {
        ...
        let out = run_with_timeout("netsh", &["wlan", "show", "interfaces"], Duration::from_millis(800))?;
            ...
            for line in out.lines() {
                ...
                if l.contains("Signal level=") {
                    ...
                    if let Some(pos) = l.find("Signal level=") {
                        ...
                        if let Ok(dbm) = v.parse::<i32>() { return Some(dbm); }
                    }
                }
            }
        }
        // fallback: nmcli -f IN-USE,SIGNAL dev wifi
        if let Some(out) = run_with_timeout("nmcli", &["-f", "IN-USE,SIGNAL", "dev", "wifi"], Duration::from_millis(800)) {
            // Найдем активную строку с '*' и возьмем сигнал (%) → конвертируем
            for line in out.lines() {
                if line.contains('*') {
                    let parts: Vec<&str> = line.split_whitespace().collect();
                    if let Some(sig_str) = parts.last() {
                        if let Ok(p) = sig_str.parse::<i32>() { return Some((p / 2) - 100); }
                    }
                }
            }
        }
                ...
                for line in out.lines() {
                    ...
                    if l.starts_with("agrCtlRSSI:") {
                        if let Some(v) = l.split(':').nth(1) {
                            if let Ok(dbm) = v.trim().parse::<i32>() { return Some(dbm); }
                        }
                    }
                }
            }
...
pub fn wifi_quality_label(dbm: i32) -> &'static str {
    if dbm >= -50 { "отличный" }
    else if dbm >= -60 { "хороший" }
    else if dbm >= -70 { "средний" }
    else { "слабый" }
}
...
pub fn ethernet_link_status() -> Option<String> {
    #[cfg(target_os = "linux")]
    {
        ...
        for ifaddr in ifaces {
            ...
            if is_loop { continue; }
            let lower = name.to_lowercase();
            let is_wifi = lower.starts_with("wl") || lower.starts_with("wlan") || lower.starts_with("wifi");
            if !is_wifi { names.push(name.to_string()); }
        }
        names.sort();
        names.dedup();
        for name in names {
            ...
            if let Ok(text) = fs::read_to_string(&path) {
                if let Ok(mbps) = text.trim().parse::<i32>() {
                    if mbps >= 1000 { return Some("1 Gbps".to_string()); }
                    if mbps > 0 { return Some(format!("{} Mbps", mbps)); }
                }
            }
        }
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(out) = run_with_timeout("wmic", &["nic", "where", "(NetEnabled=true)", "get", "Name,Speed", "/FORMAT:CSV"], Duration::from_millis(1000)) {
            // Ищем максимальную скорость
            let mut best: Option<u128> = None;
            for line in out.lines() {
                let l = line.trim();
                if l.is_empty() || l.starts_with("Node,") { continue; }
            ...
            if let Some(bps) = best {
                ...
                if bps >= 1_000_000_000 { return Some("1 Gbps".into()); }
                if bps >= 100_000_000 { return Some("100 Mbps".into()); }
                return Some("активен".into());
            }
        }
        return Some("активен".into());
...
pub fn wifi_ssid() -> Option<String> {
    #[cfg(target_os = "windows")]
    {
        if let Some(out) = run_with_timeout("netsh", &["wlan", "show", "interfaces"], Duration::from_millis(800)) {
            for line in out.lines() {
                let l = line.trim();
                let lower = l.to_lowercase();
                // Пропускаем BSSID
                if lower.starts_with("bssid") { continue; }
...
pub fn adapter_description() -> Option<String> {
    #[cfg(target_os = "windows")]
    {
        ...
        if let Some(out) = run_with_timeout("wmic", &["nic", "where", "(NetEnabled=true)", "get", "Name", "/VALUE"], Duration::from_millis(1000)) {
            for line in out.lines() {
                let l = line.trim();
