...
import svgPaths from "../imports/svg-nfe8ucn0br";

interface DetailedPathCardProps {
  onNavigateBack: () => void;
}
...
function Wrapper({ onNavigateBack }: { onNavigateBack: () => void }) {
  return (
    <div className="content-stretch flex items-start justify-between relative shrink-0 w-full" data-name="Wrapper">
      <Icon onClick={onNavigateBack} />
      <Icon1 onClick={onNavigateBack} />
    </div>
  );
}
...
function LoaderIcon({ isChecking }: { isChecking: boolean }) {
  return (
    <div className={`relative shrink-0 size-[16px] ${isChecking ? 'animate-spin' : ''}`} data-name="loader">
      <div className="absolute inset-0" data-name="Vector">
        <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 32 32">
          <g id="Vector"></g>
        </svg>
...
function Node3({ isChecking }: { isChecking: boolean }) {
  return (
    <div className="content-stretch flex gap-[8px] items-center relative shrink-0 w-full" data-name="Node">
      {isChecking ? <LoaderIcon isChecking={isChecking} /> : <CircleCheckIcon />}
...
function Wrapper4({ isChecking }: { isChecking: boolean }) {
  return (
    <div className="content-stretch flex flex-col gap-[6px] items-start relative shrink-0 w-full" data-name="Wrapper">
      <Node3 isChecking={isChecking} />
      <Text3 />
    </div>
  );
}

function Frame({ isChecking }: { isChecking: boolean }) {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-full">
      <Wrapper1 />
      <Wrapper2 />
      <Wrapper3 />
      <Wrapper4 isChecking={isChecking} />
    </div>
  );
}

function Info({ isChecking }: { isChecking: boolean }) {
  return (
    <div className="basis-0 bg-[#f6f3ee] grow min-h-px min-w-px relative rounded-[12px] shrink-0 w-full" data-name="Info">
      <div className="flex flex-col items-center size-full">
        <div className="box-border content-stretch flex flex-col gap-[8px] items-center px-[16px] py-[12px] relative size-full">
          <Frame isChecking={isChecking} />
        </div>
      </div>
    </div>
  );
...
function Bottom({ isChecking }: { isChecking: boolean }) {
  return (
    <div className="basis-0 content-stretch flex flex-col gap-[16px] grow items-start min-h-px min-w-px relative shrink-0 w-full" data-name="Bottom">
...
export default function DetailedPathCard({ onNavigateBack }: DetailedPathCardProps) {
  const [isChecking, setIsChecking] = useState(true);

  useEffect(() => {
    // Simulate network check after 2 seconds
    ...
    <div className="bg-[#fffeff] relative rounded-[12px] size-full" data-name="Card">
      ...
      <div className="flex flex-col items-center size-full">
        <div className="box-border content-stretch flex flex-col gap-[16px] items-center p-[16px] relative size-full">
          <Wrapper onNavigateBack={onNavigateBack} />
          <Bottom isChecking={isChecking} />
          <Wrapper5 />
        </div>
      </div>
    </div>
