...
type Language = 'ru' | 'en' | 'de' | 'es' | 'fr' | 'it';
...
function LanguageOption({ name, isActive, onClick }: { name: string; isActive?: boolean; onClick: () => void }) {
  return (
...
function Content({ selectedLanguage, onLanguageChange }: { selectedLanguage: Language; onLanguageChange: (lang: Language) => void }) {
  return (
    <div className="basis-0 content-stretch flex flex-col gap-[16px] grow items-start min-h-px min-w-px relative shrink-0 w-full">
      <div className="content-stretch flex gap-[8px] items-center relative shrink-0 w-full">
        <p className="basis-0 font-['Inter:Semi_Bold',_sans-serif] font-semibold grow leading-[1.2] min-h-px min-w-px not-italic relative shrink-0 text-[24px] text-neutral-950">Язык</p>
      </div>

      <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full">
        <LanguageOption 
          name="Deutsch" 
          isActive={selectedLanguage === 'de'} 
        ...
        <LanguageOption 
          name="Italiano" 
          isActive={selectedLanguage === 'it'} 
          onClick={() => onLanguageChange('it')} 
        />
        <LanguageOption 
          name="Русский" 
          isActive={selectedLanguage === 'ru'} 
...
export default function LanguageScreen({ onNavigate }: LanguageScreenProps) {
    ...
    <div className="bg-white relative rounded-[12px] size-full">
      ...
      <div className="flex flex-col items-center size-full">
        <div className="box-border content-stretch flex flex-col gap-[16px] items-center p-[16px] relative size-full">
          ...
          <Content selectedLanguage={selectedLanguage} onLanguageChange={setSelectedLanguage} />
