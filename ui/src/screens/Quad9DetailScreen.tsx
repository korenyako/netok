...
export function Quad9DetailScreen({ onBack }: Quad9DetailScreenProps) {
  ...
  const [selectedVariant, setSelectedVariant] = useState<Quad9Variant>('recommended');
      ...
      try {
        ...
        if (provider.type === 'Quad9') {
          ...
          if (variant === 'Recommended') {
            ...
            setSelectedVariant('secured_ecs');
          } else if (variant === 'Unsecured') {
            setCurrentVariant('unsecured');
            setSelectedVariant('unsecured');
          }
        }
      } catch (err) {
  ...
  const handleApply = async () => {
    try {
      ...
      const apiVariant: ApiQuad9Variant =
        selectedVariant === 'recommended' ? 'Recommended' :
        selectedVariant === 'secured_ecs' ? 'SecuredEcs' :
        'Unsecured';
